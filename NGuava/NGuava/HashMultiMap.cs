...
namespace NGuava
{
    public class HashMultiMap<K, V> : IMultiMap<K, V>
    {
        ...
        public bool ContainsKeyValuePair(KeyValuePair<K, V> pair)
        {
            Preconditions.CheckNotNullArgument(pair.Key, "key is null");
            var isContains = false;
            if (dictionary.ContainsKey(pair.Key))
            {
                var values = dictionary[pair.Key];
        ...
        public int CountPairs
        {
            ...
            private set { amountOfPairs = value; }
        }
        ...
        public void AddAll(IMultiMap<K, V> map)
        {
            ...
            {
                foreach (var value in map.Get(key))
                {
                    Add(key, value);
                }
            }
        ...
        public override bool Equals(object obj)
        {
            ...
            {
                var other = (HashMultiMap<K, V>) obj;
                answer = true;
                foreach (var key in Keys)
                {
                }
